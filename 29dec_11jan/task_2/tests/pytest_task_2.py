...
def test_remove_another_subject():
    ...
    with pytest.raises(ValueError, match="Такого предмета Литература нет, укажите другой предмет"):
        Sergey.remove_subject("Литература")
        

def test_remove_nonexistent_subject():
    Sergey = Student("Sergey", 2)
    with pytest.raises(ValueError, match="Такого предмета Физика нет, укажите другой предмет"):
...
def test_get_gpa():
    ...
    assert Konstantin.get_gpa() == 4.25 # По всем оценкам Константина 4 + 5 + 3 + 5 = 17 / 4 = 4.25
